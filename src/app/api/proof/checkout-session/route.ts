...
import { bookingPayloadSchema, BookingGuardrailError, VehicleAvailabilityError } from "@/lib/booking-payload";
...
export async function POST(request: Request) {
  ...
  if (!isStripeConfigured()) {
    return NextResponse.json(
      { error: "Stripe is not configured for proof checkout sessions." },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      booking: {
        ...
        paymentCheckoutSessionId: result.booking.paymentCheckoutSessionId,
