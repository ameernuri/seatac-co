...
type CheckoutFieldErrorKey =
 |"customerEmail"|"customerFirstName"|"customerLastName"|"customerPhone"|"customerPhoneVerified"|"customerPolicyAgreed";
...
function RideAdditionCard({
 ...
 checked
 ?"border-[#2d6a4f]/30 bg-[#2d6a4f]/8":"border-[#2d6a4f]/10 bg-white",
 )}
 >
 ...
 <button
 type="button"onClick={() => onQuantityChange(Math.max(quantity - 1, 0))}
 disabled={quantity <= 0}
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Remove ${extra.label.toLowerCase()}`}
 >
 ...
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Add ${extra.label.toLowerCase()}`}
 >
 ...
 return (
 <button
 type="button"onClick={() => onQuantityChange(checked ? 0 : 1)}
 className={cn("rounded-xl border p-3 text-left transition",
 checked
 ?"border-[#2d6a4f]/30 bg-[#2d6a4f]/8":"border-[#2d6a4f]/10 bg-white hover:border-[#2d6a4f]/20 hover:bg-[#f8f7f4]",
 )}
 >
 <div className="flex items-start justify-between gap-3">
 <div className="min-w-0 flex-1">
...
function BookingDateField({
 ...
 <PopoverTrigger
 className={cn("flex h-12 w-full items-center justify-between rounded-xl border border-[#2d6a4f]/15 bg-white px-4 text-base font-medium text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4]",
 ...
 <PopoverContent
 align="start"className="w-auto rounded-xl border-[#2d6a4f]/15 bg-white p-2 shadow-[0_20px_60px_rgba(45,106,79,0.15)]">
 <Calendar
...
function BookingTimeField({
 ...
 <button
 type="button"onClick={() => previousValue && onChange(previousValue)}
 disabled={!previousValue}
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Earlier ${label.toLowerCase()}`}
 >
 <Minus className="size-4"/>
 </button>
 ...
 <button
 type="button"onClick={() => nextValue && onChange(nextValue)}
 disabled={!nextValue}
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Later ${label.toLowerCase()}`}
 >
 <Plus className="size-4"/>
 </button>
 </div>
...
function BookingCounterField({
 ...
 <div className="flex h-12 items-center gap-2 rounded-xl border border-[#2d6a4f]/15 bg-white p-2">
 <button
 type="button"onClick={() => previousValue !== null && onChange(previousValue)}
 disabled={!canDecrease}
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Decrease ${label.toLowerCase()}`}
 >
 <Minus className="size-4"/>
 </button>
 ...
 <button
 type="button"onClick={() => nextValue !== null && onChange(nextValue)}
 disabled={!canIncrease}
 className="grid size-9 place-items-center rounded-full border border-[#2d6a4f]/15 bg-white text-[#1a3d34] transition hover:border-[#2d6a4f]/30 hover:bg-[#f8f7f4] disabled:cursor-not-allowed disabled:opacity-40"aria-label={`Increase ${label.toLowerCase()}`}
 >
 <Plus className="size-4"/>
 </button>
 </div>
...
export function ReserveWizard({
 ...
 useEffect(() => {
  setSelectedExtras((current) =>
   current.filter((key) => availableExtraKeys.has(key)),
  );
 }, [availableExtraKeys]);
 ...
 ? selectedVehicleStatus.reasonType ==="available": availableVehicles.some((vehicle) => vehicle.id === selectedVehicleId);

 const effectiveServiceMode: ServiceMode =
 selectedPricingType ==="hourly"?"hourly": selectedRoute?.mode ==="corporate"?"corporate":"airport";
 const tripTypeLabel =
 ...
 tripType ==="hourly"|| tripType ==="event"? pickupAddress ||"Hourly service area": bookingRouteName;
 const checkoutPickupPreview = formatAddressPreview(pickupAddress);
 const checkoutDropoffPreview =
 tripType ==="hourly"|| tripType ==="event"? `${hoursRequested} requested hour${hoursRequested ==="1"?"":"s"}`
 ...
 }, [allowFlatRate, bookingConstraints, minimumStep]);
 ...
 hoursRequested:
 tripType ==="hourly"|| tripType ==="event"? Number(hoursRequested)
 : null,
 }),
 });
 const data = await response.json().catch(() => ({}));
 ...
 lockedPricingType ?? selectedPricingType,
 bookingConstraints,
 (lockedPricingType ?? selectedPricingType) ==="flat",
 { allowFlatRate },
 );
 ...
 ? status.reasonType ==="schedule"?"Unavailable for this time": status.reasonType ==="inventory"?"Fleet already committed":"Available now": availabilityLoading
 ?"Checking live availability": availabilityError
 ?"Availability unavailable":"Waiting for availability";
 ...
 ?"No vehicle fits this party size.": availabilityLoading
 ?"Checking live availability.": availabilityError
 ? availabilityError
 : !vehicleStatuses
 ?"Live availability is not ready yet.": !selectedVehicle
 ?"Choose a vehicle.": !selectedVehicleMatchesFit
 ?"Choose a vehicle that fits this party size.": !selectedVehicleIsAvailable
 ? selectedVehicleStatus?.reason ??"That vehicle is unavailable for this time.": null;
 const summaryRows = [
 {
 label:"Route",
 value: routePreviewLabel,
 detail:
 tripType ==="flat"? routeConfidenceLabel
 : tripType ==="distance"?"Custom route": `${hoursRequested} requested hour${hoursRequested ==="1"?"":"s"}`,
 onEdit: () => jumpToStep(1),
 },
 {
 label:"Time",
 ...
 <Link
 href="/account"className="text-[#0d5c48] underline underline-offset-4"onClick={(event) => event.stopPropagation()}
 >
 profile
 </Link>
 .
 ...
 <Link
 href="/privacy"target="_blank"rel="noreferrer"className="text-[#0d5c48] underline underline-offset-4"onClick={(event) => event.stopPropagation()}
 >
 privacy policy
 </Link>{""}
 and{""}
 <Link
 href="/sms-policy"target="_blank"rel="noreferrer"className="text-[#0d5c48] underline underline-offset-4"onClick={(event) => event.stopPropagation()}
 >
 SMS policy
 </Link>
 .
 ...
 const clampedTargetStep = Math.max(targetStep, minimumStep) as Step;

 if (clampedTargetStep <= step) {
 setStep(clampedTargetStep);
 return;
 }
 ...
 aria-label="Pricing type"className="w-full rounded-full border border-[#2d6a4f]/12 bg-[#f7f8f5] p-1"itemClassName="flex-1 rounded-full"/>
 </div>
 ...
 <GoogleAddressInput
 id="compact-pickup-address"value={pickupAddress}
 onChange={handlePickupAddressChange}
 onResolved={setPickupPlace}
 placeholder="Enter pick up address"suggestions={pickupSuggestions}
 ...
 <GoogleAddressInput
 id="compact-dropoff-address"value={dropoffAddress}
 onChange={handleDropoffAddressChange}
 onResolved={setDropoffPlace}
 placeholder="Enter drop off address"suggestions={dropoffSuggestions}
 className="booking-control h-12 rounded-xl px-4 text-base border-[#2d6a4f]/15 bg-white text-[#1a3d34]"/>
 ...
 placeholder="Select Date"disabled={pickupDateDisabled}
 />
 <BookingTimeField
 label="Pick up time"options={pickupTimeOptions}
 value={pickupTime}
 ...
 <div className="mt-4">
 <BookingCounterField
 label="Hours requested"value={Number(hoursRequested)}
 options={hourlyHourOptions}
 singularLabel="hour"onChange={(value) => setHoursRequested(String(value))}
 />
 </div>
 )}
 ...
 className="h-12 rounded-xl border-[#2d6a4f]/15 bg-white px-4 text-base text-[#1a3d34] placeholder:text-[#8aa398]"placeholder="Example: AS342 or DL1287"/>
 </div>
 {scheduleValidationMessage ? (
 ...
 <Checkbox
 id="return-trip"checked={returnTrip}
 ...
 placeholder="Select Date"disabled={returnDateDisabled}
 />
 <BookingTimeField
 label="Return time"options={returnTimeOptions}
 value={returnTime}
 ...
 <div className="grid gap-4 sm:grid-cols-2">
 <BookingCounterField
 label="Passengers"value={Number(passengers)}
 options={Array.from({ length: 12 }, (_, index) => index + 1)}
 singularLabel="passenger"onChange={(value) => handlePassengersChange(String(value))}
 />

 <BookingCounterField
 label="Checked bags"value={Number(bags)}
 options={Array.from({ length: 13 }, (_, index) => index)}
 singularLabel="checked bag"onChange={(value) => handleBagsChange(String(value))}
 />
 </div>
 ...
 <button
 key={vehicle.id}
 type="button"disabled={!isAvailable || isPendingAvailability}
 onClick={() => isAvailable && setSelectedVehicleId(vehicle.id)}
 className={cn("relative rounded-2xl border p-5 text-left transition",
 isPendingAvailability
 ?"border-[#2d6a4f]/10 bg-white opacity-75": isAvailable
 ? selectedVehicle?.id === vehicle.id
 ?"border-[#2d6a4f]/30 bg-[#2d6a4f]/8":"border-[#2d6a4f]/10 bg-white hover:border-[#2d6a4f]/20 hover:bg-[#f8f7f4]":"border-[#2d6a4f]/5 bg-[#f8f7f4]/50 opacity-70",
 )}
 >
 ...
 <div className="min-w-0 flex-1">
 <p
 className={cn("font-sans text-[0.65rem] uppercase tracking-[0.2em]",
 ...
 <Input
 id="checkout-first-name-compact"name="given-name"autoComplete="given-name"value={customerFirstName}
 onChange={(event) => {
 setCustomerFirstName(event.target.value);
 clearCheckoutError("customerFirstName");
 }}
 onBlur={() => validateCheckoutFields({ showToast: false })}
 className={cn("h-12 rounded-xl bg-white px-4 text-base text-[#1a3d34] placeholder:text-[#8aa398]",
 checkoutErrors.customerFirstName
 ?"border-rose-300 focus-visible:ring-rose-200":"border-[#2d6a4f]/15",
 )}
 ...
 <Input
 id="checkout-last-name-compact"name="family-name"autoComplete="family-name"value={customerLastName}
 onChange={(event) => {
 ...
 <Input
 id="checkout-email-compact"name="email"type="email"autoComplete="email"value={customerEmail}
 onChange={(event) => {
 ...
 <ClientAccountForm
 variant="checkout"account={clientAccount}
 name={customerName}
 email={customerEmail}
 phone={customerPhone}
 ...
 className="min-h-28 rounded-xl border-[#2d6a4f]/15 bg-white px-4 py-3 text-base text-[#1a3d34] placeholder:text-[#8aa398]"placeholder="Client name, venue timing, gate code, or special entry notes"/>
 </div>
 ...
 {step > minimumStep && (
 <Button
 type="button"variant="outline"onClick={() => setStep(Math.max(step - 1, minimumStep) as Step)}
 ...
 <div className="space-y-2">
 <Button
 type="button"onClick={submitBooking}
 disabled={submitting}
 className="h-11 rounded-full bg-[#2d6a4f] px-6 text-sm font-semibold text-white hover:bg-[#2d6a4f]/90">
 {submitting
 ?"Redirecting...": pricing
 ...
 return (
 <div
 data-booking-theme="seatac"className={cn("theme-seatac grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_21rem] pb-32 md:pb-0",
 ...
 <Button
 type="button"variant="outline"onClick={handleStartFreshBooking}
 ...
 <div
 className="relative flex min-w-max items-start gap-6 lg:grid lg:min-w-0 lg:gap-4"style={{ gridTemplateColumns: `repeat(${visibleStepMeta.length}, minmax(0, 1fr))` }}
 >
 <div
 className="absolute hidden h-px bg-[#2d6a4f]/12 lg:block"style={{ left: stepRailInset, right: stepRailInset, top:"3.1rem"}}
 />
 {visibleStepMeta.map((item) => (
 <button
 key={item.id}
 type="button"onClick={() => jumpToStep(item.id)}
 ...
 <span
 className={cn("relative z-10 grid size-11 place-items-center rounded-full border bg-white text-sm font-semibold transition",
 step === item.id
 ?"border-[#2d6a4f] bg-[#2d6a4f] text-white shadow-[0_8px_22px_rgba(45,106,79,0.2)]": item.id < step
 ?"border-[#2d6a4f]/28 text-[#2d6a4f]":"border-[#2d6a4f]/15 text-[#8aa398]",
 )}
 >
 ...
 <div className="min-w-0 rounded-[2.5rem] bg-white p-8 lg:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)] border border-slate-100">
 ...
 <GoogleAddressInput
 id="pickup-address"value={pickupAddress}
 onChange={handlePickupAddressChange}
 onResolved={setPickupPlace}
 placeholder="Enter pick up address"suggestions={pickupSuggestions}
 ...
 <GoogleAddressInput
 id="dropoff-address"value={dropoffAddress}
 onChange={handleDropoffAddressChange}
 onResolved={setDropoffPlace}
 placeholder="Enter drop off address"suggestions={dropoffSuggestions}
 className="booking-control h-14 rounded-2xl px-4 text-base border-[#2d6a4f]/15 bg-white text-[#1a3d34]"/>
 ...
 <GoogleAddressInput
 id="pickup-address-desktop"value={pickupAddress}
 onChange={handlePickupAddressChange}
 onResolved={setPickupPlace}
 placeholder="Enter pick up address"suggestions={pickupSuggestions}
 ...
 <div className="min-w-0 flex-[1_1_0%]">
 <GoogleAddressInput
 id="dropoff-address-desktop"value={dropoffAddress}
 onChange={handleDropoffAddressChange}
 onResolved={setDropoffPlace}
 placeholder="Enter drop off address"suggestions={dropoffSuggestions}
 className="h-14 w-full rounded-none border-0 bg-transparent px-4 text-base text-[#1a3d34] shadow-none focus-visible:z-10"/>
 ...
 />
 <BookingTimeField
 label="Pickup time"options={pickupTimeOptions}
 value={pickupTime}
 onChange={handlePickupTimeChange}
 />
 ...
 value={flightNumber}
 onChange={(event) => setFlightNumber(event.target.value)}
 className="h-14 rounded-2xl border-[#2d6a4f]/15 bg-white px-4 text-base text-[#1a3d34]"placeholder="Example: AS342 or DL1287"/>
 </div>
 ...
 <div className="flex items-center gap-3">
 <Checkbox
 id="return-trip-desktop"checked={returnTrip}
 onCheckedChange={(checked) => handleReturnTripChange(Boolean(checked))}
 className="size-5 border-[#2d6a4f]/20 data-checked:bg-[#2d6a4f] data-checked:text-white"/>
 ...
 type="button"disabled={!isAvailable || isPendingAvailability}
 onClick={() => isAvailable && setSelectedVehicleId(vehicle.id)}
 className={cn("relative self-start overflow-hidden rounded-[1.7rem] border text-left transition",
 isPendingAvailability
 ?"border-[#2d6a4f]/10 bg-white opacity-75": isAvailable
 ? selectedVehicle?.id === vehicle.id
 ?"border-[#2d6a4f] bg-[#eef7f2] shadow-[0_20px_42px_rgba(45,106,79,0.24)] ring-2 ring-[#2d6a4f]/22 md:pb-4":"border-[#2d6a4f]/10 bg-white hover:border-[#2d6a4f]/20 hover:bg-[#f8f7f4] md:mt-4":"border-[#2d6a4f]/10 bg-[#f8f7f4]/50 opacity-80",
 )}
 >
 {selectedVehicle?.id === vehicle.id && isAvailable && (
 ...
 <div
 className={cn("relative h-[12.75rem] overflow-hidden rounded-[inherit] sm:h-[13.5rem] md:h-auto md:overflow-visible",
 !isAvailable &&"grayscale",
 )}
 >
 <div
 className={cn("absolute inset-0 bg-cover md:relative md:h-44",
 selectedVehicle?.id === vehicle.id
 ...
 <div
 className={cn("absolute inset-x-0 bottom-0 z-10 space-y-1.5 p-3 text-white sm:p-3.5 md:static md:space-y-4 md:bg-transparent md:p-5 md:text-inherit",
 selectedVehicle?.id === vehicle.id
 ?"text-white":"text-white/88 md:text-[#1a3d34]/88",
 )}
 >
 ...
 className={cn(
 selectedVehicle?.id === vehicle.id
 ?"text-white/82 md:text-[#5a7a6e]":"text-white/76 md:text-[#5a7a6e]/84",
 )}
 >
 Base
 </span>
 ...
 className={cn("font-medium",
 selectedVehicle?.id === vehicle.id
 ?"text-white md:text-[#1a3d34]":"text-white md:text-[#1a3d34]",
 )}
 >
 {formatCurrency(quotePreview.baseFare)}
 </span>
 ...
 <div
 className={cn("border-t px-0 pt-4 text-sm",
 status?.reasonType ==="schedule"?"border-amber-500/20 text-amber-800":"border-[#2d6a4f]/10 text-[#5a7a6e]",
 )}
 >
 ...
 <Input
 id="checkout-first-name"name="given-name"autoComplete="given-name"value={customerFirstName}
 onChange={(event) => {
 setCustomerFirstName(event.target.value);
 clearCheckoutError("customerFirstName");
 }}
 onBlur={() => validateCheckoutFields({ showToast: false })}
 className={cn("h-14 rounded-2xl bg-white px-4 text-base text-[#1a3d34]",
 checkoutErrors.customerFirstName
 ?"border-rose-300 focus-visible:ring-rose-200":"border-[#2d6a4f]/15",
 )}
 />
 ...
 <Input
 id="checkout-last-name"name="family-name"autoComplete="family-name"value={customerLastName}
 onChange={(event) => {
 setCustomerLastName(event.target.value);
 clearCheckoutError("customerLastName");
 }}
 ...
 <Input
 id="checkout-email"name="email"type="email"autoComplete="email"value={customerEmail}
 onChange={(event) => {
 setCustomerEmail(event.target.value);
 clearCheckoutError("customerEmail");
 }}
 ...
 value={notes}
 onChange={(event) => setNotes(event.target.value)}
 className="min-h-32 rounded-[1.7rem] border-[#2d6a4f]/15 bg-white px-4 py-3 text-base text-[#1a3d34]"placeholder="Client name, venue timing, gate code, or special entry notes"/>
 </div>
 </div>
 ...
 <div className="mt-8 space-y-4 border-t border-[#2d6a4f]/10 pt-6">
 {renderCheckoutSmsConsent("inline")}
 <div className="space-y-2">
 <label
 htmlFor="customer-policy-agreed"className="flex cursor-pointer items-start gap-3 py-1">
 <Checkbox
 id="customer-policy-agreed"checked={customerPolicyAgreed}
 onCheckedChange={(checked) => {
 ...
 Review the{""}
 <Link
 href="/terms"target="_blank"rel="noreferrer"className="text-[#0d5c48] underline underline-offset-4"onClick={(event) => event.stopPropagation()}
 >
 terms
 </Link>{""}
 and{""}
 ...
 <div className="mt-8 hidden flex-wrap items-center justify-end gap-4 border-t border-[#2d6a4f]/10 pt-6 lg:flex">
 <div className="flex gap-3">
 ...
 {landingOnly ? (
 <Button
 type="button"disabled={!stepOneReady}
 onClick={() => {
 if (!validateStep(1)) {
 return;
 }
 ...
 <Button
 type="button"disabled={step === 2 && Boolean(stepTwoBlockingMessage)}
 onClick={() => {
 if (!validateStep(step)) {
 return;
 }
 ...
 {!landingOnly ? (
 <aside className="hidden space-y-5 lg:sticky lg:top-28 lg:self-start lg:block">
 ...
 <div
 key={row.label}
 className={cn("group flex items-start gap-3 py-2.5",
 )}
 >
 <div className="flex shrink-0 items-center gap-2">
 <span className="text-sm text-[#5a7a6e]">{row.label}</span>
 <button
 type="button"onClick={row.onEdit}
      ...
      <div className="flex items-end justify-between gap-4">
              ...
              {pickupAddress || dropoffAddress ? (
                <>
                  <div className="flex items-center gap-3">
                    ...
                    {bags && (
                      <span className="flex items-center gap-1">
                        <Luggage className="size-3 opacity-70" />
                        {bags}
                      </span>
                    )}
                  </div>
                ...
                <span className="block w-full text-[#7d9388]">Select trip details</span>
              )}
            </div>
           <span className="text-[0.6rem] font-bold uppercase tracking-[0.18em] text-[#7d9388]">
             Estimated total
           </span>
           <span className="mt-0.5 font-sans text-[1.35rem] font-extrabold leading-none text-[#1a3d34]">
             {sidebarPriceLabel}
           </span>
         </button>
       </DrawerTrigger>
       <DrawerContent className="bg-white">
         <DrawerHeader className="border-b border-[#2d6a4f]/10 pb-4">
           <div className="flex items-center justify-between gap-4 px-1">
             <DrawerTitle className="text-xl font-bold text-[#1a3d34]">Trip Summary</DrawerTitle>
             <div className="text-right">
               <span className="block text-[0.6rem] font-bold uppercase tracking-[0.18em] text-[#7d9388]">
                 Estimated total
               </span>
               <span className="font-sans text-[1.35rem] font-extrabold leading-none text-[#1a3d34]">
                 {sidebarPriceLabel}
               </span>
             </div>
           </div>
         </DrawerHeader>
         <div className="max-h-[60vh] space-y-6 overflow-y-auto px-5 py-6">
           <RouteMapCard
             compact
             embedded
             pickupAddress={pickupAddress}
           ...
           <div className="space-y-0">
             {summaryRows
               .filter((row) => row.label !== "Route")
               .map((row) => (
               <div key={row.label} className="flex items-start gap-3 border-b border-[#2d6a4f]/5 py-3 last:border-0">
                 <span className="w-16 text-sm font-medium text-[#5a7a6e]">{row.label}</span>
                 <div className="ml-auto min-w-0 text-right">
                   <span className="block text-sm font-semibold text-[#1a3d34]">{row.value}</span>
                   {row.detail && <span className="block text-sm text-[#8aa398]">{row.detail}</span>}
                 </div>
               </div>
             ))}
           </div>
         </div>
         <DrawerFooter className="border-t border-[#2d6a4f]/10 pb-8 pt-4">
             ...
             {step > minimumStep && step < 3 && (
               <DrawerClose asChild>
                 <Button
                   ...
                   className="booking-secondary-button flex size-12 shrink-0 items-center justify-center rounded-full p-0"
                   aria-label="Back"
                 >
             ...
             {step < 3 ? (
               <DrawerClose asChild>
                 <Button
                   ...
                   className="booking-primary-button flex h-12 flex-1 items-center justify-center gap-1.5 rounded-full px-5"
                 >
                   {activeStep.cta} <ChevronRight className="size-4" />
                 </Button>
               </DrawerClose>
               ...
               <Button
                 ...
                 disabled={submitting}
                 className="booking-primary-button flex h-12 flex-1 items-center justify-center gap-1.5 rounded-full px-6 font-semibold"
               >
       ...
       {step < 3 ? (
         <Button
           ...
           className="booking-primary-button flex h-12 items-center gap-1.5 rounded-full pl-5 pr-4"
         >
           {activeStep.cta} <ChevronRight className="size-4" />
         </Button>
       ) : (
         <Button
           ...
           onClick={submitBooking}
           disabled={submitting}
           className="booking-primary-button flex h-12 items-center gap-1.5 rounded-full pl-6 pr-5 font-semibold"
         >
           {submitting ? "Redirecting..." : "Pay now"} <ChevronRight className="size-4" />
         </Button>
        )}
